...
prm_kern = PSFRegionMap.from_geojson(
    str(img_dir / f"{field_miri}-f444w_kernel_f{miri_filt}w.geojson")
)
...
(
    bg_miri,
    ivar_miri,
) = get_bg_and_ivar(sci_miri, wht_miri, bg_filter_sigma=64.0)
...
fits.writeto(
    out_root / (str(out_root) + "_residual.fits"),
    res[0],
    fits.getheader(fsci_miri),
    overwrite=True,
)
...
scene_table.write(
    out_root / (str(out_root) + f"_scene_catalog.csv"), format="ascii.csv", overwrite=True
)
