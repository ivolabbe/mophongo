...
class GlobalAstroFitter(SparseFitter):
    ...
    * K Chebyshev terms →  global coefficients α_k, β_k
    ...
    def __init__(
        ...
        print(
            f"GlobalAstroFitter: {self.n_flux} templates and {np.sum(good)} with S/N >= {self.config.snr_thresh_astrom} used for astrometry"
        )
        if not np.any(good):
    ...
    def _collapse(self):
        ...
        S = csr_matrix(
            (np.ones_like(inv), (np.arange(inv.size), inv)), shape=(inv.size, uniq.size)
        )  # selector
    ...
    def _apply_shifts(self) -> None:
        ...
        for i, tmpl in enumerate(self._orig_templates):
            ...
            tmpl.data[:] = nd_shift(
                tmpl.data, (-dy, -dx), order=3, mode="constant", cval=0.0, prefilter=True
            )
