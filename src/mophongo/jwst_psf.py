...
def psf_grid_from_wcs_csv(
    ...
    *,
    ...
    for det in det_list:
        ...
        outfile = '_'.join(filter(None, (prefix, det, filt, postfix))) + '.fits'
...
#            outfile    = outfile,
    ...
    return grids if len(grids) > 1 else grids[0]
...
# ─────────────────────────────────────────────────────────────────────
# Helper: convert any meta key to a legal FITS keyword (≤8 char, A–Z0–9)
# ─────────────────────────────────────────────────────────────────────
def _fits_key(name: str) -> str:
    key = ''.join(c for c in name.upper() if c.isalnum())[:8]
    return key or 'METAKEY'
