...
def plot_lw_coadd_diagnostic(
    bands: list[dict[str, Any]],
    coadd_sci: np.ndarray,
    coadd_wht: np.ndarray,
    *,
    out_path: str | Path,
    ...
    """Per-band sci + coadd stamps for the LW detection coadd.

    One row per centre. Columns: each input band's science cutout,
    coadded science cutout, coadded weight cutout.

    Parameters
    ----------
    bands
        Same list passed to :func:`mophongo.utils.lw_detection_coadd`.
        ``sci`` may be an array or a FITS path.
    coadd_sci, coadd_wht
        Outputs from :func:`mophongo.utils.lw_detection_coadd`.
    out_path
        Output PNG path.
    centers
        ``[(y, x), ...]`` in pixels. Default: image centre.
    half_size
    ...
    finite = np.isfinite(coadd_sci) & (coadd_sci != 0)
    if finite.any():
        vmin, vmax = np.percentile(coadd_sci[finite], list(sci_percentiles))
    else:
        vmin, vmax = 0.0, 1.0
    norm = ImageNormalize(vmin=vmin, vmax=vmax, stretch=AsinhStretch(a=asinh_a))

    def _load(x: Any) -> np.ndarray:
        if isinstance(x, np.ndarray):
            return x
        return fits.getdata(str(x))

    arrays = [
        (b.get("name", f"band{i}"), _load(b["sci"]))
    ...
    for r, (yc, xc) in enumerate(centers):
        y0 = max(0, int(round(yc)) - half_size)
        y1 = min(H, int(round(yc)) + half_size + 1)
        x0 = max(0, int(round(xc)) - half_size)
        x1 = min(W, int(round(xc)) + half_size + 1)
        for c, (name, arr) in enumerate(arrays):
            sub = arr[y0:y1, x0:x1]
            axes[r, c].imshow(sub, origin="upper", cmap="gray", norm=norm)
            axes[r, c].set_title(name)
            axes[r, c].set_axis_off()
        axes[r, -2].imshow(
            coadd_sci[y0:y1, x0:x1], origin="upper", cmap="gray", norm=norm,
        )
        axes[r, -2].set_title("coadd sci")
        axes[r, -2].set_axis_off()
        wcut = coadd_wht[y0:y1, x0:x1]
        wfin = wcut[np.isfinite(wcut)]
        if wfin.size:
            wvmin, wvmax = np.percentile(wfin, [1, 99])
        else:
            wvmin, wvmax = 0.0, 1.0
        axes[r, -1].imshow(
            wcut, origin="upper", cmap="viridis", vmin=wvmin, vmax=wvmax,
        )
        axes[r, -1].set_title("coadd wht")
